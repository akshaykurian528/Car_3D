...
loader.load("./assets/car_3d_model.glb", (gltf) => {
    model = gltf.scene;
    model.traverse((node) => {
        if (node.isMesh) {
            if (node.material) {
    ...
    // Create ScrollTrigger for the .info section and animate the model along with it
    const scrollTriggerInstance = ScrollTrigger.create({
        trigger: ".info",
        start: "top top",    // Start when .info section's top hits the top of the viewport
        ...
        onUpdate: (self) => {
            ...
            gsap.to(model.position, {
                y: progress * 25,  // Adjust this to control how far the model moves up
